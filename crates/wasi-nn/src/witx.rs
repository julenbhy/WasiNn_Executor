...
impl WasiNnCtx {
    ...
    /// Get a reference to the backend for a given encoding.
    /// Added by Julen Bohoyo
    pub fn get_output_tensor(
        &mut self,
        exec_id: u32,
        index: u32,
    ) -> Result<crate::wit::types::Tensor> {
        if let Some(exec_context) = self.executions.get_mut(exec_id.into()) {
    ...
    /// Set the input tensor for a given execution context.
    /// Added by Julen Bohoyo
    pub fn set_input_tensor(
        &mut self,
        exec_id: u32,
        index: u32,
        tensor: crate::wit::types::Tensor,
    ) -> Result<()> {
        if let Some(exec_context) = self.executions.get_mut(exec_id.into()) {
            exec_context.set_input(Id::Index(index), &tensor)?;
            Ok(())
        } else {
            Err(UsageError::InvalidGraphHandle.into())
        }
